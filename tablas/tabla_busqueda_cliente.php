...
$sql = "SELECT `codigo`, `id`, `nombre`, `telefono`, `tipo` FROM `clientes` WHERE `nombre` LIKE '%$busqueda%' OR `id` LIKE '%$busqueda%' OR `telefono` LIKE '%$busqueda%' ORDER BY `nombre` ASC LIMIT $start,10";
$result = mysqli_query($conexion, $sql);
			...
			<a href="javascript:seleccionar_cliente('<?php echo $mostrar[0] ?>','<?php echo $mostrar[1] ?>','<?php echo $nombre ?>','<?php echo $telefono ?>')">
				<li class="list-group-item-bg list-group-item d-flex justify-content-between align-items-center p-1">
					<div class="row align-items-center text-truncate">
						<span class="h6 mb-0 text-capitalize"><?php echo $nombre . ' ' . $vip ?></span>
		...
		<div class="col-sm-12 col-md-12 text-center pt-2">
			<ul class="pagination">
				...
				<li class="paginate_button page-item <?php echo $disabled ?>">
					<a href="javascript:$('#tabla_busqueda_cliente').load('tablas/tabla_busqueda_cliente.php/?page=<?php echo $page - 1 ?>&input_buscar=<?php echo $busqueda ?>', function(){cerrar_loader();});" class="page-link">
						<</a>
				</li>
				<?php
				...
				?>
						<li class="paginate_button page-item <?php echo $active ?>">
							<a href="javascript:$('#tabla_busqueda_cliente').load('tablas/tabla_busqueda_cliente.php/?page=<?php echo $i ?>&input_buscar=<?php echo $busqueda ?>', function(){cerrar_loader();});" class="page-link"><?php echo $i ?></a>
						</li>
				<?php
					}
				}
				...
				?>
				<li class="paginate_button page-item <?php echo $disabled ?>">
					<a href="javascript:$('#tabla_busqueda_cliente').load('tablas/tabla_busqueda_cliente.php/?page=<?php echo $page + 1 ?>&input_buscar=<?php echo $busqueda ?>', function(){cerrar_loader();});" class="page-link">></a>
				</li>
			</ul>
		</div>
	</div>
