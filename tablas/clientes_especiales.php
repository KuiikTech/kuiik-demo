...
if (isset($_GET['input_buscar'])) {
	...
	$sql = "SELECT `codigo`, `id`, `nombre`, `telefono`, `direccion`, `ciudad`, `correo`, `fecha_registro` FROM `clientes` WHERE tipo = 'Especial' AND (`nombre` LIKE '%$busqueda%' OR `id` LIKE '%$busqueda%' OR `telefono` LIKE '%$busqueda%') ORDER BY `nombre` ASC";
	$result = mysqli_query($conexion, $sql);
} else {
	$sql = "SELECT `codigo`, `id`, `nombre`, `telefono`, `direccion`, `ciudad`, `correo`, `fecha_registro` FROM `clientes` WHERE tipo = 'Especial' ORDER BY `nombre` ASC LIMIT 50";
	$result = mysqli_query($conexion, $sql);
	$busqueda = '';
}

$nombre_tabla = 'Clientes Especiales encontrados';
?>
<!-- Tabla Productos -->
<div class="card">
				...
				<tbody class="overflow-auto">
					...
					?>
						<tr role="row" class="odd">
							...
							<td class="p-1"><b><?php echo $nombre ?></b></td>
							<td class="p-1 text-right"><?php echo $telefono ?></td>
							<td class="p-1"><?php echo $direccion ?></td>
							<td class="p-0 text-center">
								<button class="btn btn-sm btn-outline-primary btn-round p-1" data-bs-toggle="modal" data-bs-target="#Modal_Ver" onclick="document.getElementById('div_loader').style.display = 'block';$('#div_detalles_cliente').load('paginas/detalles/detalles_cliente_especial.php/?cod_cliente=<?php echo $codigo ?>', function(){cerrar_loader();});">
