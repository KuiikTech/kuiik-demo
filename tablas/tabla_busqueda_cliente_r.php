...
$sql = "SELECT `codigo`, `id`, `nombre`, `telefono`, `fecha_registro` FROM `clientes` WHERE `nombre` LIKE '%$busqueda%' OR `id` LIKE '%$busqueda%' OR `telefono` LIKE '%$busqueda%' ORDER BY `nombre` ASC LIMIT $start,10";
$result=mysqli_query($conexion,$sql);
...
<div class="container px-3 m-0">
	...
	<ul class="list-group list-group-flush">
		...
		{ 
			...
			$telefono = ucwords(str_ireplace('))//','</mark>', $telefono));
			?>
			<a href="javascript:seleccionar_cliente_r('<?php echo $mostrar[0] ?>','<?php echo $mostrar[1] ?>','<?php echo $nombre ?>','<?php echo $telefono ?>')">
		...
		<div class="col-sm-12 col-md-12 text-center pt-2">
			<ul class="pagination">
				...
				<li class="paginate_button page-item <?php echo $disabled ?>">
					<a href="javascript:$('#tabla_busqueda_cliente_r').load('tablas/tabla_busqueda_cliente_r.php/?page=<?php echo $page-1 ?>&input_buscar=<?php echo $busqueda ?>', function(){cerrar_loader();});" class="page-link"><</a>
				</li>
				<?php 
				...
				{ 
					...
					if($i>($page-$cant_pages2) && $i<($page+$cant_pages))
					{
						...
						<li class="paginate_button page-item <?php echo $active ?>">
							<a href="javascript:$('#tabla_busqueda_cliente_r').load('tablas/tabla_busqueda_cliente_r.php/?page=<?php echo $i ?>&input_buscar=<?php echo $busqueda ?>', function(){cerrar_loader();});" class="page-link"><?php echo $i ?></a>
						</li>
						<?php 
					}
				}
