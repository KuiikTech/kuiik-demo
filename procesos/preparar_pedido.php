...
if (isset($_SESSION['usuario_restaurante'])) {
	...
	if (isset($productos_pedido[$num_item])) {
		if ($productos_pedido[$num_item]['estado'] == 'PENDIENTE') {
			$productos_pedido[$num_item]['estado'] = 'PREPARANDO';
			$productos_pedido[$num_item]['fecha_preparando'] = $fecha_h;
			$productos_pedido[$num_item]['preparador'] = $usuario;
			...
			if ($verificacion == 1) {
				if ($respuesta == 'VENTA') {
					...
					foreach ($productos_venta as $key => $value) {
						if ($value['code'] == $code) {
							$productos_venta[$key]['estado'] = 'PREPARANDO';
							$productos_venta[$key]['fecha_preparando'] = $fecha_h;
						}
					}
					...
					foreach ($productos_mesa as $key => $value) {
						if ($value['code'] == $code) {
							$productos_mesa[$key]['estado'] = 'PREPARANDO';
							$productos_mesa[$key]['fecha_preparando'] = $fecha_h;
						}
					}
			...
			$verificacion = 'El producto ya se encuentra en Preparación/Despachado';
	} else
