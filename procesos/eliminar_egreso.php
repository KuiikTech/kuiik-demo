	...
	if($rol == 'Administrador')
	{
		...
		if($estado == 'ABIERTA')
		{
			...
			foreach ($egresos as $j => $egreso)
			{
				if($j != $item)
				{
					$nuevos_egresos[$pos] = $egreso;
					$pos ++;
				}
			}

			if($pos >1)
				$nuevos_egresos = json_encode($nuevos_egresos,JSON_UNESCAPED_UNICODE);
			else
				$nuevos_egresos = '';
			...
			$verificacion = 'No se eliminó el egreso. La caja NO se encuentra abierta';
	}
	else
		$verificacion = 'Solo los administradores pueden borrar Gastos';
}
else
	$verificacion = 'Reload';
