		...
		if ($mostrar_mesa[2] != '') {
			if ($verificacion == 1) {
				...
				foreach ($productos_mesa as $i => $producto) {
					...
					$codigos_productos[$i]['cod_producto'] = $cod_producto;
					$codigos_productos[$i]['cant'] = $producto['cant'];
					$codigos_productos[$i]['movimientos'] = $movimientos;
				}
				...
				if ($verificacion == 1) {
					...
					if ($verificacion == 1) {
						foreach ($productos_areas as $p => $datos_area) {
							...
							$sql_pedido = "INSERT INTO `pedidos`(`codigo`, `productos`, `mesa`, `solicitante`, `fecha_registro`, `fecha_envio`, `estado`, `area`, `fecha_entrega`, `respuesta`) VALUES (
							'$codigo',
							'$productos_pedido',
							'$cod_venta',
							'$usuario',
